...
data Immediate = Immediate !Int64 | LabelRef !Label | ImmMem !Int64 | LabelMem !Label
...
db :: MonadCode m => Text -> m Label
db str = do
    tellCodeLine $ Raw $ ".section .rodata"
    lbl <- label
    tellCodeLine $ Raw $ "\t.asciz \"" <> str <> "\""
    tellCodeLine $ Raw $ ".section .text"
    pure lbl
...
compileOp = \case
    ...
    ImmOp (LabelMem l) -> "$" <> Text.pack (show l)
    ImmOp (ImmMem n) -> Text.pack (show n)
